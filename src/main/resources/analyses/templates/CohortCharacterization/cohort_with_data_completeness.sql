...
WHERE EXTRACT(YEAR FROM C.drug_exposure_start_date) - P.year_of_birth >= 65
...
WHERE (OP.OBSERVATION_PERIOD_START_DATE + 180*INTERVAL'1 day') <= P.START_DATE AND (P.START_DATE + 0*INTERVAL'1 day') <= OP.OBSERVATION_PERIOD_END_DATE AND P.ordinal = 1
-- End Primary Events
...
CREATE TEMP TABLE collapse_constructor_input
...
FROM
collapse_constructor_input
;

-- era constructor
CREATE TEMP TABLE collapse_constructor_output
